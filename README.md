...
## v0.9.2 — 구조적 강제 (Gemma 4 31B에서 실측 검증)

Ollama Cloud의 **gemma4:31b**에서 실측한 결과, v0.9.1은 한 가지 실패가 남아 있었다: 모델은 전체 하강과 Final Report를 잘 만들지만, 모든 **Real / Descend** 판정을 *근거(reasoning)만으로* ("사람 판단이 필요하다") 내리고, 자기검사가 요구한 후보 대체 스테이션을 한 번도 쓰지 않았다. 자기검사 '질문'만으로는 부족했고 — Gemma 4 31B는 출력 *템플릿*을 충실히 따르므로 — v0.9.2는 후보 시도를 **필수 템플릿 필드**로 만든다:

- 라인 로그의 **Open-and-Split Result**가 스테이션당 3줄 블록으로 변경: `Candidate replacement(s)` → `Why it fails` → `Verdict`. Candidate 줄 없는 판정은 FAIL — "Real"을 쓰려면 먼저 구체적 대체 스테이션을 시도해야만 하는 구조.
- 원시연산 5, §3 출력 규율, §6 최종 감사 모두 "근거만으로 판정"을 금지하고, 후보가 **보이는 출력**에 나와야 함을 요구(숨은 사고가 아님) — Gemma 4의 알려진 사고-출력 괴리를 직접 보정.
- worked 예시(§4)도 새 3줄 템플릿으로 재정렬해 few-shot이 정확한 형태를 가르치도록 함.

실측: 동일 문제에서 `Candidate` 등장 0회 → 3회, 모든 판정이 3줄 구조를 갖춤. 공장 라인 용어와 8 원시연산 구조는 그대로.

---

## v0.9.1 — 중형 인스트럭트 모델(Gemma 3 27B급) 대응 강화

v0.9.0에서 이미 소형 모델 자급자족 언어를 갖췄지만, **Gemma 3 27B** 같은 중형 인스트럭트 모델은 세 가지 문서화된 방식으로 실행을 무너뜨렸습니다: (1) 기본 간결 — 층별 로그를 요약/통합하고 자기검사를 건너뜀; (2) '도움되는 척 단축경로' — 대체 시도 없이 모든 스테이션을 **Real**로 찍음; (3) 이른 부정 제약 누락. v0.9.1은 Gemma 3의 알려진 동작에 근거한 정밀 보강을 추가합니다:

- **구체적 worked 예시**(§4) — 2-층 하강 실례. Gemma는 산문 규칙보다 예시를 훨씬 잘 따르므로 few-shot이 가장 효과 큰 수단.
- **출력 규율 노트**(§3) — 층 병합/생략 금지, 증명 없는 Layer-0 바닥 선언 금지, "Real" 판정 전 후보 대체 스테이션 명시 의무화.
- **자기검사 강화**(원시연산 5b / 6 / 7) — "답한 뒤 행동; Yes면 FAIL: redo" 형태로 부정문 오독 방지.
- **최종 자기감사**(§6) — 가장 잘 누락되는 제약을 맨 끝 체크리스트로 재배치(Gemma는 이른 부정 제약을 놓치므로 핵심은 끝에).

공장 라인 용어와 8 원시연산 구조는 그대로이며, 재설계가 아닌 견고화입니다. 설계 목표 유지: 소형 모델부터 작동, 이제 중형 인스트럭트 모델에서 거짓 바닥점이 더 줄어듭니다.
...
5. **열어-쪼개기 검사 (Open-and-split test)** — 인간 판단 스테이션마다 **먼저 구체적 기계/자동조정 대체 스테이션과 그것이 실패하는 한 줄 이유를 보이는 출력에 명시**합니다.
* 후보가 성공하면 **가짜 인간 스테이션**입니다. 라인에서 제거하고 **대체 스테이션 이름을 명시**합니다.
* 후보가 안 맞고 하위 스테이션으로 쪼개지면 **하강**합니다.
* 후보가 안 맞고 재오픈해도 동일 판단이면 **진짜 인간 스테이션**입니다. 라인에 유지합니다. "사람 판단이 필요하다"는 근거만으로 판정하는 것은 금지.
*자기검사*: 보이는 후보+실패 이유 줄 없이 Real/Fake/Descend 판정을 내렸는가? $\rightarrow$ 실패.


6. **바닥점 (Bottom point)** — 남은 모든 인간 스테이션이 **후보 대체 시도를 거친** 열어-쪼개기 검사를 통과했을 때만(인상이 아님) 바닥점입니다. 이 시점에서 하강을 중단합니다.
*자기검사*: "상상이 안 된다"이거나 스테이션이 "사람 것처럼 보인다"는 이유로 중단했는가? $\rightarrow$ 거짓 바닥점. 각 스테이션에서 실제 열어-쪼개기를 수행해야 함.
7. **라인 로그 (Line log)** — 방문한 층마다 한 항목씩(스케치, 3분류 결과, 인간 스테이션 I/O 계약, 스테이션별 3줄 열어-쪼개기 결과) 다음 층으로 하강하기 **전에** 기록합니다. 층을 병합·요약·생략하지 않습니다.
*자기검사*: 로그 항목 없이 하강했거나, 층을 병합/생략했는가? $\rightarrow$ 실패.
...
4. **열어-쪼개기 + 로그 + 하강 또는 바닥 정지** — 각 인간 스테이션마다 후보 기계/자동조정 대체 스테이션과 실패 이유를 (보이는 출력에) 쓰고 1~3단계를 재귀적으로 적용합니다.
* 후보가 성공하면 **가짜 인간 스테이션**으로 판정하여 제거하고 대체 공정명을 기록합니다.
* 후보가 안 맞고 쪼개지면 **하강**합니다.
* 후보가 안 맞고 동일 판단이 계속 반환되면 **진짜 인간 스테이션**으로 유지합니다. 각 스테이션의 결과는 **3줄 라인 로그**에 기록되며, 쪼개 가능한 스테이션이 더 이상 없으면 바닥점에 도달하여 하강을 종료합니다.
...
- Open‑and‑split result:
  - [B_n,1]:
    - Candidate replacement(s) (기계/자동조정, 또는 "none fits"): …
    - Why it/they fail (한 줄씩): …
    - Verdict: fake (replaced by: …) | real (kept) | descend → layer k
...
* **거짓 바닥점 금지** — 후보 대체 시도를 거친 실제 열어-쪼개기 검사 없이 스테이션을 "진짜"/"바닥점"이라 선언하지 마십시오("상상이 안 된다"는 이유로 진짜가 되지 않습니다). 반대로, 대체 스테이션 이름 지정 없이 "가짜"라 선언하는 것도 금지됩니다.
* **근거만 판정 금지** — 모든 Real/Fake/Descend 판정 앞에 보이는 로그에 후보 기계/자동조정 대체 스테이션과 실패 이유가 와야 합니다. "사람 판단이 필요하다"는 증거가 아닙니다 — 먼저 구체적 대체를 시도하십시오. (Gemma 4 31B 실측에서 1순위 단축경로였음.)
